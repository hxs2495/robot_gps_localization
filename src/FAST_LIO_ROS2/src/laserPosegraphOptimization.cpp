...
void pubPath( void )
{
    ...
    for (int node_idx=0; node_idx < recentIdxUpdated; node_idx++) // -1 is just delayed visualization (because sometimes mutexed while adding(push_back) a new one)
    {
        ...
        odomAftPGOthis.header.stamp = get_ros_time(keyframeTimes.at(node_idx));
...
void pubMap(void)
{
    int SKIP_FRAMES = 2; // sparse odom visulalization to save computations 
    int counter = 0;
...
int main(int argc, char **argv)
{
  ...
  scMaximumRadius = node->get_parameter("sc_max_radius").get_parameter_value().get<double>();
	...
	std::thread viz_map {process_viz_map}; // visualization - odom (low frequency because it is heavy)
