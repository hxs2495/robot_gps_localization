...
public:
  ...
  : Node("fusion_path_node")
  {
    ...
    odom_sub_ = create_subscription<nav_msgs::msg::Odometry>(
      odom_topic,
      rclcpp::QoS(10),
      std::bind(&FusionPathNode::odomCallback, this, std::placeholders::_1));

    RCLCPP_INFO(
      get_logger(),
      "全局融合路径节点已启动，订阅: %s，发布: %s",
      odom_topic.c_str(),
      path_topic.c_str());
  }
...
private:
  void odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg)
  {
    ...
    const std::string frame_id =
      path_frame_id_.empty() ? msg->header.frame_id : path_frame_id_;

    if (frame_id.empty()) {
      RCLCPP_WARN_THROTTLE(
        get_logger(), *get_clock(), 3000,
        "里程计header.frame_id为空，发布的Path也将没有坐标系。");
    }

    if (!path_.header.frame_id.empty() && path_.header.frame_id != frame_id) {
      RCLCPP_WARN(
        get_logger(),
        "Path坐标系从%s变为%s，已清空旧路径。",
  ...
  bool shouldAppendPose(const geometry_msgs::msg::PoseStamped & pose) const
  {
  ...
  void trimPath()
  {
    ...
    while (static_cast<int>(path_.poses.size()) > max_path_size_) {
